    ...
    <div class="w-4/5 md:w-3/5 flex flex-col items-center gap-8 mx-auto">
        ...
        {{-- <section class="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-2">
            @foreach ($charities as $charity)
                <div class="flex items-center justify-center h-64 w-64 bg-cover"
                    style="background-image: url('{{ asset($charity->preview_image) }}')">
                    <a class="text-white font-bold" href="{{ route('charity.show', $charity->id) }}"
                        wire:navigate>{{ $charity->name }}</a>
                </div>
            @endforeach
        </section> --}}
        <div class="">
            <div class="mx-auto max-w-7xl lg:px-8">
                <ul role="list"
                    class="mx-auto mt-20 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 sm:grid-cols-2 lg:mx-0 lg:max-w-none lg:grid-cols-2">
                    @foreach ($charities as $charity)
                        <li>
